...
export function SimplifiedCampaignPage() {
  ...
  const exportToCsv = async () => {
    ...
    try {
      // Debug: Log full leads structure with detailed custom variables info
      console.log('=== CSV EXPORT DEBUG ===');
      console.log('Total leads:', leads.length);
      console.log('Full leads data (first lead):', leads[0]);

      // Collect all unique custom variable names from leads
      const customVariableNames = new Set<string>();
      let leadsWithCustomVars = 0;
      let totalCustomVarsFound = 0;
      ...
      leads.forEach((lead, leadIndex) => {
        ...
        leadDataArray.forEach((data: any, dataIndex: number) => {
          // Check if custom_variables exists and log its type and structure
          const hasCustomVars = data?.custom_variables !== undefined && data?.custom_variables !== null;
          const customVarsType = typeof data?.custom_variables;
          const isObject = customVarsType === 'object' && !Array.isArray(data?.custom_variables);

          console.log(`Lead ${leadIndex}, Data ${dataIndex} [${data?.email}]:`, {
            email: data?.email,
            has_personalized_message: !!data?.personalized_message,
            personalized_message_length: data?.personalized_message?.length || 0,
            has_custom_variables: hasCustomVars,
            custom_variables_type: customVarsType,
            custom_variables_is_object: isObject,
            custom_variables: data?.custom_variables,
            custom_variables_keys: hasCustomVars && isObject ? Object.keys(data.custom_variables) : 'N/A',
            all_data_keys: Object.keys(data || {})
          });

          if (hasCustomVars && isObject) {
            leadsWithCustomVars++;
            const keys = Object.keys(data.custom_variables);
            totalCustomVarsFound += keys.length;
            keys.forEach(key => customVariableNames.add(key));
          }
        });
      });

      console.log('=== CUSTOM VARIABLES SUMMARY ===');
      console.log('Unique custom variable names found:', Array.from(customVariableNames));
      console.log('Number of leads with custom variables:', leadsWithCustomVars);
      console.log('Total custom variable instances:', totalCustomVarsFound);
      ...
      // Show export success with custom variables info
      const customVarCount = customVariableNames.size;
      const exportMessage = customVarCount > 0
        ? `Exported ${csvRows.length} leads with ${customVarCount} custom variable${customVarCount > 1 ? 's' : ''}: ${Array.from(customVariableNames).join(', ')}`
        : `Exported ${csvRows.length} leads. No custom variables found in campaign data.`;
      ...
      // If no custom variables found but user might expect them, show warning
      if (customVarCount === 0) {
        console.warn('⚠️ No custom variables found in any leads. Check backend logs to verify if custom variables were generated by AI.');
      }
    } catch (error) {
